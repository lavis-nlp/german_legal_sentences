...
In recent years, many efforts have been made to make German court decisions freely available to the public. Although a decent number of decisions can now be found on the Internet, it can be difficult for the layperson to find relevant information in them.

With this dataset, we want to accelerate research on semantic search technologies in German domain-specific language. We hope that emerging search technologies will enable the layperson to find relevant information without knowing the specific terms used by the professionals. Furthermore, we think that the lessons learned due the work with this dataset can be applied to other domains and languages.
...
## Build Positive Sentence Pairs // Employ semantic matching heuristics
...
*Hugging Face Datasets* makes accessing scientific Datasets very fast and easy. We uploaded 
...
#TODO
Make clear: ranking challenged, index, potentially billions of sentences